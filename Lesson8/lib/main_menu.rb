...
class MainMenu
  ...
  def wagons_menu_train
    ...
    loop do
      ...
      case gets.to_i
      when 1 then add_wagon_train(train)
      when 2 then remove_wagon_train(train)
      when 3 then wagons_list(train)
      when 4 then manage_wagons_menu(train.wagons)
      when 5 then break
      else enter_another_value
      end
    end
  ...
  def select_train
    ...
    if selected_train.nil?
      puts ENTER_CORRECTION_NUMBER_TRAIN # 'Введите правильный номер поезда и повторите попытку.'
    else
  ...
  def create_wagon_train(train, number)
    ...
    volume = gets.to_i
    train.type == :cargo ? CargoWagon.new(number, volume) : PassengerWagon.new(number)
  rescue RuntimeError => e
    puts e.message
    retry
  end
  ...
  def trains_list
    @trains.each_with_index do |train, index|
      print "#{index} - Наименование поезда - #{train.number}, тип: #{train.type}, количество вагонов: #{train.wagons.size}"
    end
  end
